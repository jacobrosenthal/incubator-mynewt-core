 ...
 *             it can be a nrfx_adc_config_t, to override the default
 *             configuration.
 *
...
#if 0
    if (buf2) {
        rc = nrfx_adc_buffer_convert((nrf_adc_value_t *) buf2,
                buf_len);
        if (rc != NRFX_SUCCESS) {
            goto err;
        }
